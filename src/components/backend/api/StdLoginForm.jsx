...
function StdLoginForm({LoggedIn}) {
  ...
  const SubmitForm = (e) => {
    ...
    axios.post('http://localhost/odform/sd/studentselectbyid.php', sentData).then(res => {
      if(res.data.status) {
        LoggedIn(res.data)
      } else {
  ...
  return (
    <div className="container w-100 d-flex flex-column justify-content-center align-items-center" style={Height}>
      <form onSubmit={SubmitForm} className='col-md-3'>
        <h2 className='my-4'>Studnet SignIn</h2>
        <div className='my-2'>
